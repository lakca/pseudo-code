...
const{NODE:NODE,TOKEN:TOKEN,ENTITY_NODE_TOKEN:ENTITY_NODE_TOKEN}=require("./def");function parse(n){let E=-1,N="",e="";const t=[];let i=null,f=null,r=0,O=0;const u=n.length;function D(){return e+=N,N=n[++E],{index:E,token:N}}function o(n){for(;n--;)D()}function s(N){let e=1;const t=function(){let N=E;return function(){return n[++N]}}();for(;t()===N;)e++;return e}function c(n){return f={s:"",t:n,v:(i=f||i)?i.v:0},t.push(f),f}function T(n){_(),n&&c(n),i=f||i,f=null}function K(n){f||c(NODE.RAW_INFER),n===TOKEN.ESCAPE?r+=1:(_(),f.s+=n)}function _(){r>0&&(f.s=TOKEN.ESCAPE.repeat(r),r=0)}function l(){return f||c(NODE.RAW_INFER),O+=Math.floor(r/2),r%2==0?(f.s+=TOKEN.ESCAPE.repeat(r/2),r=0,!1):(f.s+=TOKEN.ESCAPE.repeat(Math.floor(r/2)),r=0,!0)}function A(){for(c(NODE.MARK),function(){for(let n=t.length;n--&&!(t[n].t===NODE.NEWLINE||t[n].s.indexOf("\n")>-1);)ENTITY_NODE_TOKEN[t[n].t]||(O+=1);f.s+="&nbsp".repeat(O),f._=O,O=0}();D().index<u;)if(!(C()||W()||p()||M()))if(N===TOKEN.MARK_END){if(!l())return T(NODE.MARK_END),!0;K(N)}else K(N);return!0}function R(){if(N===TOKEN.OR&&!l())return c(NODE.OR),K(N),T(),!0}function a(){if(N===TOKEN.AND&&!l())return c(NODE.AND),K(N),T(),!0}function C(){if(N===TOKEN.NEWLINE)return c(NODE.NEWLINE),T(),e="",!0}function F(){if(N===TOKEN.RAW&&!l())return function(){for(c(NODE.RAW);D().index<u;)if(!C())if(N===TOKEN.RAW_END){if(!l())return T(NODE.RAW_END),!0;K(N)}else K(N);return!0}()}function I(){if(N===TOKEN.DEF){const n=t[t.length-2];if((!f||f.t===NODE.NEWLINE||!n||n.t===NODE.NEWLINE&&""===f.s.trim())&&!l())return function(){for(c(NODE.DEF);D().index<u;)if(!F()){if(TOKEN.SEP.indexOf(N)>-1){if(C())continue;return T(),K(N),!0}K(N)}return!0}()}}function M(){if(N===TOKEN.LINK&&!l())return function(){for(c(NODE.LINK);D().index<u;)if(!F()&&!W())if(N===TOKEN.LINK_END){if(!l())return T(NODE.LINK_END),!0;K(N)}else K(N);return!0}()}function d(){if(N===TOKEN.META&&!l())return function(){for(c(NODE.META);D().index<u;)if(!(F()||M()||W()||R()||a()))if(N===TOKEN.META_END){if(!l())return T(NODE.META_END),!0;K(N)}else K(N);return!0}()}function W(){if(N===TOKEN.CODE){const n=s(TOKEN.CODE);if(o(n-1),!l())return function(n){for(c(NODE.CODE);D().index<u;)if(N===TOKEN.CODE_END){if(s(TOKEN.CODE_END)>=n)return o(n-1),T(NODE.CODE_END),!0;K(N)}else K(N);return!0}(n)}}function p(){if(N===TOKEN.STRONG&&!l())return function(){for(c(NODE.STRONG);D().index<u;)if(N===TOKEN.STRONG_END){if(!l())return T(NODE.STRONG_END),!0;K(N)}else K(N);return!0}()}function L(){if(N===TOKEN.FUNC&&!l())return function(){if(!f||f.t!==NODE.RAW&&f.t!==NODE.RAW_INFER)return;const n=f.s.match(/^(.*?)(\S+)$/);if(n){for(f.s=n[1],c(NODE.FUNC),c(NODE.FUNC_NAME),K(n[2]),c(NODE.FUNC_LEFT_EDGE),K(N),c(NODE.FUNC_PARAM);D().index<u;)if(!(W()||M()||d()||R()||a()||F()))if(N!==TOKEN.FUNC_PARAM_SEP||l())if(N===TOKEN.FUNC_END){if(!l())return T(NODE.FUNC_PARAM_END),T(NODE.FUNC_RIGHT_EDGE),K(N),T(NODE.FUNC_END),!0;K(N)}else K(N);else c(NODE.FUNC_PARAM_END),c(NODE.FUNC_PARAM_SEP),K(N),c(NODE.FUNC_PARAM),T();return!0}}()}function $(){if(N===TOKEN.MARK&&!l())return A()}for(;D().index<u;){if(N===TOKEN.RAW){if(F())continue}else if(N===TOKEN.NEWLINE){if(C())continue}else if(N===TOKEN.DEF){if(I())continue}else if(N===TOKEN.OR){if(R())continue}else if(N===TOKEN.AND){if(a())continue}else if(N===TOKEN.CODE){if(W())continue}else if(N===TOKEN.STRONG){if(p())continue}else if(N===TOKEN.META){if(d())continue}else if(N===TOKEN.MARK){if($())continue}else if(N===TOKEN.LINK){if(M())continue}else if(N===TOKEN.FUNC&&L())continue;K(N)}return t}function generate(n){return console.log(n),n.reduce((n,E)=>E.t===NODE.NEWLINE?n+"<br/>":E.t===NODE.LINK?n+`<a class="${E.t}" href="#${E.s}">${E.s}`:E.t===NODE.LINK_END?n+"</a>":E.t===NODE.DEF?n+`<a class="${E.t}" href="#${E.s}" id="${E.s}">${E.s}</a>`:E.t===NODE.MARK?n+`<span class="${E.t}" spaces=${E._}>${E.s}`:E.t.endsWith("_END")?n+"</span>":NODE[E.t+"_END"]?n+`<span class="${E.t}">${E.s}`:n+`<span class="${E.t}">${E.s}</span>`,"")}module.exports={parse:parse,generate:generate};
